...
SAFE_PATH = "img/green_circle.png"
...
class GameWidget(QWidget):
    ...
    def update_grid(self):
        ...
        WIDTH = self.ms_game.board_width
        HEIGHT = self.ms_game.board_height

        solver_board = ['' for j in range(HEIGHT)]
        for i in range(HEIGHT):
            for j in range(WIDTH):
                solver_board[i] = solver_board[i] + get_symbol[info_map[i][j]]
        solver = MineSAT(solver_board)
        safe_tiles = solver.find_tiles()

        for i in range(self.ms_game.board_height):
            for j in range(self.ms_game.board_width):
                if (i+1, j+1) in safe_tiles:
                    self.grid_wgs[(i, j)].info_label(SAFE_TILE)
                else:
...
class FieldWidget(QLabel):
    ...
    def info_label(self, indicator):
        ...
        indicator : int
            ...
            12 is a mine field.
            13 is a guaranteed safe tile (as given by the solver).
        """
        if indicator in range(1, 9):
            ...
            self.id = 11
            self.setPixmap(QtGui.QPixmap(SAFE_PATH).scaled(
                    self.field_width*3, self.field_height*3))
